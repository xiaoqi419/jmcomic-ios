...
export function MoviePlayerScreen() {
  ...
  useEffect(() => {
    fetchVideoDetail(vid).then((d) => {
      setVideo(d.video);
      if (d.video?.photo) fetchImageAsDataUri(d.video.photo).then(setPhotoUri).catch(() => {});
  ...
  return (
    <SafeAreaView edges={["top"]} style={{ flex: 1, backgroundColor: '#000' }}>
      ...
      <View style={{ aspectRatio: 16 / 9, justifyContent: 'center', alignItems: 'center', backgroundColor: '#1a1a1a' }}>
        ...
        <Pressable
          onPress={() => Linking.openURL(video.full_url || video.video_src || `https://18comic.vip/video/${vid}`)}
        ...
        {video.tags?.length > 0 ? (
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 12 }}>
            {video.tags.map((tag: string, i: number) => (
              <View key={i} style={S2.tag}>
                <Text style={S2.tagText}>{tag}</Text>
              </View>
            ))}
          </View>
        ) : null}
        {video.description ? (
          <Text style={{ color: '#aaa', fontSize: FontSize.body, marginTop: 12, lineHeight: 20 }}>{video.description}</Text>
        ) : null}
      </View>
    </SafeAreaView>
  );

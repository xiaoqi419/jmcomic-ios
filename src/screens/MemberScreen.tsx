      ...
      <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={{ padding: Spacing.marginEdge, paddingBottom: 100 }}>
        ...
        <Section title={t('member.settings')} icon="settings">
          ...
          <Row label="自定义配置地址" right={
            <Pressable onPress={() => setShowConfigModal(true)} style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
              <Text style={[styles.rowValue, { fontSize: 13, maxWidth: 140 }]} numberOfLines={1}>{customConfigUrl || '未设置'}</Text>
              <MaterialIcons name="chevron-right" size={18} color={C.textTertiary} />
            </Pressable>
          } />
          <ConfigUrlModal visible={showConfigModal} onClose={() => setShowConfigModal(false)} />
